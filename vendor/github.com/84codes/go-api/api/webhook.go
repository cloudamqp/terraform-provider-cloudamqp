...
// CreateWebhook enables webhook for a specific queue
...
// ReadWebhook retrieves a specific webhook
...
// ReadWebhooks retrieces all webhooks assigned to an account.
func (api *API) ReadWebhooks(instanceID int, webhookID string) (map[string]interface{}, error) {
	data := make(map[string]interface{})
	failed := make(map[string]interface{})
...
// UpdateWebhook update the webhook with new information
func (api *API) UpdateWebhook(instanceID int, webhookID string, params map[string]interface{}) error {
	failed := make(map[string]interface{})
	log.Printf("[DEBIG] go-api::webhook::update instance ID: %d, webhookID: %s", instanceID, webhookID)
	path := fmt.Sprintf("/api/instances/%d/webhooks/%s", instanceID, webhookID)
	response, err := api.sling.New().Put(path).BodyJSON(params).Receive(nil, &failed)

	if response.StatusCode != 204 {
		return fmt.Errorf("UpdateWebhook failed, status: %v, message: %s", response.StatusCode, failed)
	}
