...
func (api *API) CreateAlarm(ctx context.Context, instanceID int, params map[string]any) (
	...
	case 201:
		...
		if id, ok := data["id"]; ok {
			...
			return nil, fmt.Errorf("invalid alarm identifier: %v", data["id"])
		}
		return data, err
	default:
		return nil,
			fmt.Errorf("failed to create alarm, status: %d, message: %s", response.StatusCode, failed)
	}
}
...
func (api *API) ListAlarms(ctx context.Context, instanceID int) ([]map[string]any, error) {
	...
	tflog.Debug(ctx, fmt.Sprintf("reuqest path: %s", path))
	response, err := api.sling.New().Get(path).Receive(&data, &failed)
	if err != nil {
		return nil, err
	}
...
func (api *API) UpdateAlarm(ctx context.Context, instanceID int, params map[string]any) error {
	...
	tflog.Debug(ctx, fmt.Sprintf("reuqest path: %s, params: %v", path, params))
	...
	case 201:
		return nil
	default:
		return fmt.Errorf("failed to update alarm, status: %d, message: %s",
