...
type RabbitMqConfigRequest struct {
	...
	MQTTMaxSessionExpiryIntervalSeconds   *MQTTMaxSessionExpiryInterval `json:"rabbitmq_mqtt.max_session_expiry_interval_seconds,omitempty"`
	...
	MessageInterceptorsTimestampOverwrite *string                       `json:"message_interceptors.incoming.set_header_timestamp.overwrite,omitempty"`
}
...
type RabbitMqConfigResponse struct {
	...
	MessageInterceptorsTimestampOverwrite string                        `json:"message_interceptors.incoming.set_header_timestamp.overwrite"`
}
...
// Custom type for MQTTMaxSessionExpiryInterval
type MQTTMaxSessionExpiryInterval struct {
	IsInfinity bool
	Value      *int64
}

func (c MQTTMaxSessionExpiryInterval) MarshalJSON() ([]byte, error) {
	if c.IsInfinity {
		return json.Marshal("infinity")
	}
	return json.Marshal(c.Value)
...
func (c *MQTTMaxSessionExpiryInterval) UnmarshalJSON(data []byte) error {
	var asString string
	if err := json.Unmarshal(data, &asString); err == nil {
		if asString == "infinity" {
			c.IsInfinity = true
	...
	return fmt.Errorf("MQTTMaxSessionExpiryInterval: invalid JSON value")
}
